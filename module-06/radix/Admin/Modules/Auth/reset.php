...
if (!empty($forgotToken)) {
    ...
    $user = getRowCount("SELECT user_id FROM radix_users WHERE user_forgot_token='{$forgotToken}'");
    if (!empty($user)) {
        $renderForm = true;
    } else {
...
if (isPost()) {
    ...
    if (empty($validationErrors)) {
        $user = first('radix_users', "user_forgot_token='{$inputToken}'", 'user_id, user_email');
        if (!empty($user)) {
            $userID = $user['user_id'];
            ...
            if (!empty($update_status)) {
                ...
                $content .= '<a href="' . getLinkAdmin('auth', 'login') . '">Truy cập vào hệ thống</a>';
